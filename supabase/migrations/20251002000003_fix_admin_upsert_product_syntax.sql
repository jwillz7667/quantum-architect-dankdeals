-- Fix SQL syntax error in admin_upsert_product function
-- Error: "invalid reference to FROM-clause entry for table 'products'"
-- On line: slug = COALESCE(EXCLUDED.slug, public.products.slug)
-- Fix: In ON CONFLICT DO UPDATE, reference existing column as just 'slug', not 'public.products.slug'
...
BEGIN
  ...
  ON CONFLICT (id) DO UPDATE
    SET
      ...
      slug = COALESCE(EXCLUDED.slug, products.slug),  -- FIXED: removed 'public.' prefix
...
'Creates or updates a product along with its variants. Requires administrator permissions. Fixed table reference syntax error.';
