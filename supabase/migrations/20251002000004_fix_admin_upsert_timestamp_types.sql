-- Fix timestamp type mismatch in admin_upsert_product function
-- Error: column "updated_at" is of type timestamp with time zone but expression is of type time with time zone
-- Issue: Used current_time (returns TIME) instead of now() (returns TIMESTAMPTZ)
-- Fix: Replace all instances of current_time variable with now() function calls
...
BEGIN
  ...
  -- Set timestamps using now() instead of storing in variable
  ...
  ON CONFLICT (id) DO UPDATE
    SET
      ...
      updated_at = now()  -- FIXED: use now() instead of current_time variable
...
'Creates or updates a product along with its variants. Requires administrator permissions. Uses now() for timestamp generation to ensure correct TIMESTAMPTZ type.';
