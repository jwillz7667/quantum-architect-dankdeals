-- Fix admin_upsert_product to properly handle TEXT variant IDs
-- The database uses TEXT for variant IDs (supports custom IDs like "pv_pf_eighth")
-- But the function was treating them as UUIDs, causing type errors
...
DECLARE
  ...
  variant_id text;  -- FIXED: Changed from uuid to text
  tracked_variant_ids text[] := ARRAY[]::text[];  -- FIXED: Changed from uuid[] to text[]
BEGIN
  ...
  LOOP
    ...
    -- FIXED: Handle TEXT variant IDs instead of UUIDs
    -- If id is provided, use it; otherwise generate a custom text ID
    variant_id := variant_record->>'id';
    IF variant_id IS NULL OR variant_id = '' THEN
      -- Generate a simple text ID (can be customized to your naming scheme)
      variant_id := 'pv_' || substr(md5(random()::text), 1, 8);
    END IF;
    ...
    WITH variant_input AS (
      ...
      FROM jsonb_to_record(variant_record) AS v(
        id text,  -- FIXED: Changed from uuid to text
        product_id uuid,
        name text,
        price numeric,
        weight_grams numeric,
...
'Creates or updates a product along with its variants. Requires administrator permissions. Properly handles TEXT variant IDs (supports custom identifiers like pv_productname_size).';
