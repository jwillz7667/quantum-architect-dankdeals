-- Complete database setup for product listings and image storage
...
CREATE TRIGGER trigger_generate_product_slug
  ...
  FOR EACH ROW
  WHEN (NEW.slug IS NULL OR NEW.slug = '' OR (TG_OP = 'UPDATE' AND OLD.name IS DISTINCT FROM NEW.name))
  EXECUTE FUNCTION public.generate_product_slug();

-- Create a function to update product search vectors for full-text search
